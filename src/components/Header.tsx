    ...
    <header className="sticky top-0 z-50 w-full bg-white/95 backdrop-blur-md border-b border-neutral-100/80 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14">
          ...
          >
            <div className="w-9 h-9 rounded-xl bg-violet-50/80 flex items-center justify-center border border-violet-200/50 shadow-xs text-violet-600 transition-transform duration-300 hover:scale-105 active:scale-95">
              <Flower className="w-5 h-5 fill-violet-100/30 text-violet-500 animate-pulse" />
            </div>
            <div className="flex flex-col">
              <span className="font-bold text-sm sm:text-base text-neutral-800 tracking-tight leading-none font-display">筑摩野中 りんどう祭</span>
              <span className="text-[8px] font-bold uppercase tracking-wider text-neutral-400 mt-1">Chikumano Junior High School</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-1.5 hover:bg-neutral-100 rounded-lg text-neutral-500 lg:hidden cursor-pointer"
            >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            className="lg:hidden border-t border-neutral-100 bg-white/98 backdrop-blur-md py-3 px-4 flex flex-col gap-2 shadow-lg overflow-hidden"
          >
            {tabs.map((tab) => (
              <button
                key={tab.id}
