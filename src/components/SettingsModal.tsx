...
import { X, Cookie, ShieldCheck, Check, RotateCcw, Globe, Sparkles, Cloud, Lock, Map } from 'lucide-react';

interface SettingsModalProps {
  onClose: () => void;
  onSavePreferences?: (choices: { functional: boolean; analytics: boolean }) => void;
}
...
export default function SettingsModal({ onClose, onSavePreferences }: SettingsModalProps) {
  ...
  const handleSave = () => {
    ...
    if (onSavePreferences) {
      onSavePreferences({ functional: consentFunctional, analytics: consentAnalytics });
    }
    setTimeout(() => {
      setIsSaved(false);
      onClose();
  ...
  return (
    <div className="fixed inset-0 z-50 bg-neutral-950/70 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white text-neutral-800 max-w-xl w-full rounded-[28px] border border-neutral-100 shadow-2xl overflow-hidden flex flex-col my-8 animate-scale-up text-left">
        {/* Header */}
        <div className="px-6 py-5 bg-gradient-to-r from-neutral-800 to-neutral-900 text-white flex items-center justify-between">
        ...
        <div className="p-6 sm:p-8 space-y-6 overflow-y-auto max-h-[70vh]">
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-black text-neutral-900 tracking-wide uppercase border-l-2 border-indigo-500 pl-2">
              プライバシー・クッキー設定
            </h3>
            
            <p className="text-[11px] sm:text-xs text-neutral-500 leading-relaxed">
              マップ設定のパーソナライズ（ズームや回転の自動保持）や、サイト利用状況統計の保持に使用するCookieをお好みに合わせて変更できます。
            </p>
            ...
            <div className="border border-neutral-200/80 rounded-2xl divide-y divide-neutral-150 overflow-hidden text-xs">
              ...
              <div className="p-4 bg-neutral-50/60 space-y-1">
                ...
                <p className="text-[10px] text-neutral-400 leading-relaxed font-sans mt-1">
                  システムのコア動作、同意ステータスの保持（この表示を二度出さない判定設定）、および口コミ投稿・レビューの読み込み確認に利用されます。
                </p>
              </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-[10px] text-neutral-400 leading-relaxed font-sans mt-0.5 block">
                      マップのズームスケールや90〜270度自立回転の角度設定を次回表示時まで状態保持するために利用されます。
                    </span>
                  </div>
                  <div className="shrink-0 flex items-center justify-end pl-4">
                    <label className="relative inline-flex items-center cursor-pointer select-none">
                      ...
                      />
                      <div className="w-10 h-5.5 bg-neutral-200 rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4.5 after:w-4.5 after:transition-all peer-checked:bg-indigo-600"></div>
                    </label>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-[10px] text-neutral-400 leading-relaxed font-sans mt-0.5 block">
                      混雑時にどのスポットが最も見られているか等のアクセス統計を記録し、今後のりんどう祭での誘導設計の改善に役立てられます（匿名データのみ）。
                    </span>
                  </div>
                  <div className="shrink-0 flex items-center justify-end pl-4">
                    <label className="relative inline-flex items-center cursor-pointer select-none">
          ...
          <div className="bg-neutral-50/80 rounded-2xl border border-neutral-200/50 p-4 space-y-2.5">
            ...
            <p className="text-[10px] text-neutral-500 leading-relaxed">
              マップ自体は「国土地理院空中写真（航空写真）」を利用し、本アプリ作成者がgemini.google.comを用いて加工・イラスト化し、Google AI StudioおよびCloudflare環境で構築された公式マップビューワです。
            </p>
        ...
        <div className="bg-neutral-50 px-6 py-4.5 border-t border-neutral-100 flex items-center justify-end gap-3 shrink-0">
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl font-bold text-xs tracking-wider text-white shadow-md flex items-center gap-1.5 transition-all cursor-pointer ${
              isSaved ? 'bg-emerald-600 shadow-emerald-600/10' : 'bg-indigo-600 hover:bg-indigo-700'
            }`}
          >
            {isSaved ? (
              <>
